...
import { 
  ArrowLeft, Terminal, Copy, Check, 
  Heart, Eye, Download, Cpu, Clock, Calendar, Box 
} from "lucide-react"
import Link from "next/link"
import { LeanFunction } from '@/types';

export function FuncPageClient({ name, defaultt }: { name: string, defaultt: LeanFunction }) {
  const supabase = createClient()
  const [isCodeCopied, setIsCodeCopied] = useState<boolean>(false)
  const [isForRunCopied, setIsForRunCopied] = useState<boolean>(false)
  const [isLiked, setIsLiked] = useState<boolean>(false)
  const [data, setData] = useState<LeanFunction>(defaultt)
